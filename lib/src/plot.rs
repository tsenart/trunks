...
/// Options for generating an HTML latency plot.
pub struct PlotOpts {
    /// Plot title.
    pub title: String,
    /// Maximum number of data points per series (LTTB downsampling threshold).
    pub threshold: usize,
}
...
/// Label function type for partitioning hits into series.
pub type Labeler = fn(&Hit) -> String;

/// Default labeler: partitions hits into "attack: OK" and "attack: ERROR".
pub fn error_labeler(hit: &Hit) -> String {
...
/// Generate an HTML latency plot from a slice of hits.
///
/// Writes a self-contained HTML page with an interactive latency scatter plot
/// to the provided writer. Uses LTTB downsampling to keep the output size
/// manageable for large datasets.
pub fn plot(
    hits: &[Hit],
    opts: &PlotOpts,
    labeler: Labeler,
    ...
    // Build per-series (label) sorted point arrays.
    let mut series_map: BTreeMap<String, Vec<(f64, f64)>> = BTreeMap::new();
    for hit in hits {
        let label = labeler(hit);
    ...
    for points in series_map.values_mut() {
        ...
        if points.len() > opts.threshold {
            *points = lttb::downsample(points, opts.threshold);
        }
    }
...
mod tests {
    ...
    fn make_hit(attack: &str, code: u16, latency_ms: u64, error: &str, ts_secs: u64) -> Hit {
        Hit {
            attack: attack.to_string(),
            seq: 0,
            code,
            timestamp: std::time::UNIX_EPOCH + std::time::Duration::from_secs(ts_secs),
    ...
    #[test]
    fn plot_empty_hits_returns_error() {
        let opts = PlotOpts::default();
        let mut buf = Vec::new();
        let result = plot(&[], &opts, error_labeler, &mut buf);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("no data"));
    }

    #[test]
    fn plot_single_hit_produces_html() {
        let hits = vec![make_hit("test", 200, 50, "", 1000)];
        let opts = PlotOpts::default();
        let mut buf = Vec::new();
        plot(&hits, &opts, error_labeler, &mut buf).unwrap();
    ...
    #[test]
    fn plot_multiple_attacks_produces_series() {
        let hits = vec![
            make_hit("a1", 200, 50, "", 1000),
            make_hit("a2", 200, 100, "", 1000),
        ];
        let opts = PlotOpts::default();
        let mut buf = Vec::new();
        plot(&hits, &opts, error_labeler, &mut buf).unwrap();
    ...
    #[test]
    fn error_labeler_categorization() {
        let ok_hit = make_hit("test", 200, 50, "", 1000);
        assert_eq!(error_labeler(&ok_hit), "test: OK");

        let err_hit = make_hit("test", 0, 50, "timeout", 1000);
        assert_eq!(error_labeler(&err_hit), "test: ERROR");
    }

    #[test]
    fn plot_with_custom_title() {
        let hits = vec![make_hit("test", 200, 50, "", 1000)];
        let opts = PlotOpts {
